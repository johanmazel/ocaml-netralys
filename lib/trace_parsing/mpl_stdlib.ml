 ...
 *
 * $Id: mpl_stdlib.ml,v 1.1 2011/07/07 12:37:40 jmazel Exp $
 *)
...
type env = {
  __bbuf: string;           (* data *)
  __btlen: int;             (* total length of data, Bytes.length bbuf *)
...
let fill_string env buf =
  let len = Bytes.length buf in
    Bytes.blit buf 0 env.__bbuf !(env.__blen) len;
    (* appended by me *)
    env.__blen:= len;
    env.__bsz <- len
    (* end appended by me *)
...
module Mpl_byte = struct
    ...
    let __unmarshal env = 
        let c = Bytes.get env.__bbuf (env.__bbase + env.__bpos) in
    ...
    let __marshal env v =
        Bytes.set env.__bbuf (env.__bbase + env.__bpos) v;
        incr_pos env 1
 
    let __unsafe_marshal env v =
        Bytes.unsafe_set env.__bbuf (env.__bbase + env.__bpos) v;
        incr_pos env 1
    ...
    let at env off =
        Bytes.get env.__bbuf (env.__bbase + off)
...
module Mpl_uint16 = struct
    ...
    let dissect fn acc env =
        ...
        |Big_endian ->
            for i = 0 to n-1 do
               let a = int_of_char (Bytes.unsafe_get env.__bbuf (bp+(i*2))) in
               let b = int_of_char (Bytes.unsafe_get env.__bbuf (bp+(i*2)+1)) in
        ...
        if amt mod 2 = 1 then begin
           let a = int_of_char (Bytes.unsafe_get env.__bbuf (bp+amt-1)) in
...
module Mpl_raw = struct
    let marshal env str =
        ...
        Bytes.blit str 0 env.__bbuf (env.__bbase + env.__bpos) len;
        incr_pos env len
    ...
    let __unsafe_marshal env str =
        let len = Bytes.length str in
        Bytes.unsafe_blit str 0 env.__bbuf (env.__bbase + env.__bpos) len;
        incr_pos env len

    let at env off len =
        Bytes.sub env.__bbuf (env.__bbase + off) len
    ...
    let blit tenv fenv =
        Bytes.blit fenv.__bbuf fenv.__bbase tenv.__bbuf
    ...
    let prettyprint s =
        ...
        for i = 0 to Bytes.length s - 1 do
            ...
            let pchar c =
               let s = Bytes.make 1 c in if Char.escaped c = s then s else "." in
            Buffer.add_string buf1 (sprintf " %02X" (int_of_char (Bytes.get s i)));
            Buffer.add_string buf2 (sprintf " %s" (pchar (Bytes.get s i)));
        done;
...
let dump_env env =
   prerr_endline (Mpl_raw.prettyprint (Bytes.sub env.__bbuf 0 !(env.__blen)))
...
module Mpl_dns_label = struct
   ...
   let dump () =
        Hashtbl.iter (fun k v -> Printf.printf "%s=%s " (Bytes.concat "." k) (match v with |None -> "X" |Some off -> string_of_int off)) marshal_labels;
        print_newline ()
   ...
   let found = Hashtbl.mem marshal_labels x in 
         if comp && found then
             ...
             fn (Bytes.length bit + 1 + am) r; (* for length of bit *)
         end
      |[] -> am + 1 in
      let sz = fn 0 s in
      sz,s
   ...
   let marshal ?(comp=false) env ((psz,t):t) =
      assert(env.__bbase + env.__bpos + psz < (Bytes.length env.__bbuf));
      ...
      let insert_string env bit x =
          ...
          Mpl_byte.__marshal env (Mpl_byte.of_int (Bytes.length bit));
   ...
   let size (x:t) = fst x
   let prettyprint t = sprintf "[%s]" (Bytes.concat "." t)
end
...
module Mpl_string32 = struct
   ...
   let size (s:t) = Bytes.length s + 4 (* for the length prefix *)
   ...
   let marshal env (t:t) : t =
      Mpl_uint32.__marshal env (Mpl_uint32.of_int (Bytes.length t));
      Mpl_raw.marshal env t;
      t
      
   let prettyprint t = sprintf "%S [%d]" t (Bytes.length t)
end
...
module Mpl_string8 = struct
   ...
   let size (s:t) = Bytes.length s + 1 (* for the length prefix *)
   ...
   let marshal env (t:t) : t =
      Mpl_byte.__marshal env (Mpl_byte.of_int (Bytes.length t));
      Mpl_raw.marshal env t;
      t
...
module Mpl_mpint = struct
        ...
        let strip_leaders leader x =
            ...
            if len = 0 then x else begin
                ...
                while !cont && !same < len do
                    if Bytes.get x !same != leader then cont := false
                    else incr same
                done;
                (* XXX if same=0 then just return it *)
                Bytes.sub x (!same) (len - !same)
            end
        in
        let x = strip_leaders '\000' s in
        let msb = Char.code (Bytes.get x 0) in
        if (Bytes.length x = 1) && (msb = 0) then ""
        else begin
    ...
    let list_of_bytes x =
        ...
        let rec explode s = 
            ...
            else (Bytes.get s 0)::(explode (Bytes.sub s 1 (Bytes.length s - 1)))
        in
        match negative with
        |false -> begin
    ...
    let prettyprint x = 
        ...
        (if neg then "-" else "") ^ (Bytes.concat "" (List.map string_of_byte x))
